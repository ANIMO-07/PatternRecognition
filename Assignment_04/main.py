...
def draw_line(ax, clf, xx, yy, **params):
...
def show_data(X_train, X_test, y_train, y_test):
...
def show_classifier(model, X, y):
    ...
    X0, X1 = X["x"], X["y"]
    xx, yy = draw_data(X0, X1)

    draw_line(ax, model, xx, yy, cmap=plt.cm.coolwarm, alpha=0.8)
...
if __name__ == "__main__":
    ...
    show_classifier(model, X_test, Y_test)
    ...
    show_classifier(clf, X_test, Y_test)

    ## Part 3
    print("\n 3. Using SVM :- \n")
    show_data(X_train, X_test, Y_train, Y_test)
    svc = SVC(kernel="linear").fit(X_train, Y_train)
    show_classifier(svc, X_test, Y_test)
